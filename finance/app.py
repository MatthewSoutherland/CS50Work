...
def buy():
    ...
    # Also insert the transactions info in the database
    else:
...
def history():
    ...
    transaction_info = db.execute(
        "SELECT type, symbol, price, shares, timestamp FROM transactions WHERE user_id = ? ORDER BY timestamp DESC", session["user_id"])
...
def sell():
    ...
    if request.method == "GET":
        ...
        # Query the symbol from the transactions database for the user
    ...
    # When user already selected their stock to sell in the drop-down list
        ...
        current_own_shares = db.execute("SELECT SUM(shares) as total_shares FROM transactions WHERE user_id = ? AND symbol = ? GROUP BY symbol",
        ...
        # Update the transactions table in database for the sell action
        db.execute("INSERT INTO transactions(user_id, type, symbol, name, shares, price) VALUES(?, ?, ?, ?, ?, ?)",
